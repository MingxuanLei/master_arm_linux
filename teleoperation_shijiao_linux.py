"""
teleoperation_shijiao_linux.py
Linux 版 7 电机同构遥操作 + 遥操作记录 + 示教回放三模式脚本。

相对于 Windows 版 teleoperation_shijiao_danxiang.py 的主要变化：
    ...
    4. 保留原有遥操作、轨迹记录、示教回放和第 7 工具电机跟随逻辑。
...
# 这里强制替换为 USBCANFD_gai.USBCANFD，避免主端/从端两个 ArmController 同时 open_device 时互相冲突。
...
CSV_FIELDS = (
    ...
    + ["master_tool", "slave_tool_target", "slave_tool_actual"]
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="七电机同构遥操作、遥操作记录与示教回放程序")
    parser.add_argument(
        "--mode",
...
def write_record_row(writer, record_start_time, loop_count, q_master, q_target, q_slave_actual, q_master_tool, q_tool_target, q_slave_tool_actual):
    row = {
