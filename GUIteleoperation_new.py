"""
GUIteleoperation_new.py
Linux 下新版弱双向遥操作 + 遥操作记录 + 示教回放的外层 GUI 控制界面。
说明：
    本文件不重复实现底层遥操作算法，而是直接调用同目录下的
    teleoperation_shijiaonew_linux.py。该后端脚本的记录文件包含
    slave_actual_v1~slave_actual_v6 和 slave_tool_actual_vel 等速度字段。
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("新版弱双向遥操作 / 速度记录 / 示教回放控制界面")
    ...
    def _build_script_group(self, parent_layout: QVBoxLayout) -> None:
        ...
        default_script = Path(__file__).resolve().parent / "teleoperation_shijiaonew_linux.py"
    ...
    def _build_status_group(self, parent_layout: QVBoxLayout) -> None:
        ...
        self.script_state_label = QLabel("调用方式：python3 -u teleoperation_shijiaonew_linux.py")
    ...
    def _build_hint_group(self, parent_layout: QVBoxLayout) -> None:
        ...
        hint = QLabel(
            "1. 先确认 teleoperation_shijiaonew_linux.py 及其依赖文件在同一目录。\n"
    ...
    def choose_script(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self,
            "选择 teleoperation_shijiaonew_linux.py",
    ...
    def start_backend(self, initial_cmd: str) -> None:
        script = Path(self.script_edit.text().strip() or "teleoperation_shijiaonew_linux.py")
